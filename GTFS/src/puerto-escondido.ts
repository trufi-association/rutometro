...
async function main() {
  console.log('Starting GTFS generation for Puerto Escondido, Mexico...');

  try {
    await osmToGtfs({
      outputFiles: {
        outputDir: path.join(__dirname, '..', 'out', 'puerto-escondido'),
        gtfs: true,
        gtfsZip: true,
        readme: true,
      },
      ...
      gtfsOptions: {
        ...
        agencyUrl: 'https://transportes-puerto-escondido.mx/',
    ...
    console.log(`📁 Output files are in: ${path.join(__dirname, '..', 'out', 'puerto-escondido')}`);
  } catch (error) {
