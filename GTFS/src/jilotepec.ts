...
async function main() {
  ...
  try {
    await osmToGtfs({
      outputFiles: {
        outputDir: path.join(__dirname, '..', 'out', 'jilotepec'),
        gtfs: true,
        gtfsZip: true,
        readme: true,
      },
      ...
      gtfsOptions: {
        ...
        agencyUrl: 'https://transportes-jilotepec.mx/',
        cityName: 'jilotepec',
        defaultCalendar: () => 'Mo-Su 06:00-22:00',
        frequencyHeadway: () => 300, // 5 minutes
        vehicleSpeed: (routeFeature: any) => {
    ...
    console.log(`📁 Output files are in: ${path.join(__dirname, '..', 'out', 'jilotepec')}`);
  } catch (error) {
